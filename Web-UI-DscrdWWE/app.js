// DscrdWWE: Resurgence - Web Configuration Interface
...
class DiscordWebhookUI {
    ...
    handleWsMessage(event) {
        ...
        if (data && data.status === 'ok' && data.variables) {
            ...
            const varName = this.globalVarName?.value || 'DWWE_WebhookPayload';
    ...
    saveToLocalStorage() {
        ...
        localStorage.setItem('dwweConfig', JSON.stringify(data));
    }

    loadFromLocalStorage() {
        try {
            const data = localStorage.getItem('dwweConfig');
            if (!data) return;
    ...
    exportJSON() { const json = this.buildJSON(); const dataStr = JSON.stringify(json, null, 2); const blob = new Blob([dataStr], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `dwwe-payload-${Date.now()}.json`; a.click(); }
    ...
    clearAll() { document.querySelectorAll('input[type="text"], input[type="number"], textarea, input[type="datetime-local"]').forEach(el => el.value = ''); document.querySelectorAll('input[type="checkbox"]').forEach(cb => cb.checked = false); if (this.embedsContainer) this.embedsContainer.innerHTML = ''; localStorage.removeItem('dwweConfig'); this.updatePreview(); this.showNotification('All data cleared','info'); }
}
